...
//
//  Notes: A generic class that contains functions to read/compile
//      shader files as well as debug data for a cube.
//
...
GLuint GLESRenderer::LoadProgram(const char *vertShaderSrc, const char *fragShaderSrc)
{
    ...
    // Note: Apparently you can delete shader reference after linking
    glDeleteShader(vertexShader);
    glDeleteShader(fragmentShader);
...
int GLESRenderer::GenWall(float scale, float **vertices, float **normals,
                         float **texCoords, int **indices)
{
    int i;
    int numVertices = 24;
